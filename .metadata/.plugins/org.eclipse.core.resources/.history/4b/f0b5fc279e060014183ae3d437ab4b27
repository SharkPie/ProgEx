...
public Collection<String[]> querys(String eusername, String epassword) {
	...
	public synchronized String[][] getenrollWindow(String eWusername, String eWpassword) {
		...
		String sql = "SELECT * FROM student_administration.Course WHERE (Course_ID NOT IN (SELECT Distinct Course_FK_ID FROM student_administration.Prerequisite_Exam) OR Course_ID IN (Select e.Course_FK_ID from student as s left join exam_current as e on s.Student_ID=e.Student_FK_ID where e.Grade_Value > 4 and s.Last_Name='"+eWusername+"' and s.Password='"+eWpassword+"') OR Course_ID IN (SELECT Distinct Course_fk_ID FROM student_administration.Prerequisite_Exam  where precondition_value in (Select e.Course_FK_ID from student as s left join exam_current as e on s.Student_ID=e.Student_FK_ID where e.Grade_Value < 5 and s.Last_Name='"+eWusername+"' and s.Password='"+eWpassword+"'))) AND (Course_ID NOT IN (SELECT Course_FK_ID FROM student_administration.Exam_Current where Student_FK_ID=1 and Grade_Value < 5))"; 
